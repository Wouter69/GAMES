...
#----------------------------------------------------------
#------INIT_SET_UP:: LOADING/WELCOME/LOGIN/REGISTER/HOME/ADS
#vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv

#CAN TURN THIS INTO A POPUP... 
...
class Home(BoxLayout):
    def __init__(self, **kw):
        ...
        self.ads_bank = ""
        self.ads_count = 0
        Clock.schedule_interval(self.on_start, 2)
    ...
    #FUNCT FOR TESTING PURPOSES ONLY
    def test_graphix(self):
        MDApp.get_running_app().root.current = 'Game'

    def on_start(self, inst):
        self.NAME = str(self.FM.read_file("SOCKET_DATA/NAME.txt"))
        name = self.NAME.split("@")
        try:
            print("HOME::ON_START():: PLAYER_DATA:: ")
            for _ in name:
                print("ITEM:: ", str(_))
            user = str(name[0]).translate(str.maketrans('','',string.punctuation))
            icon = str(name[5]).translate(str.maketrans('','',string.punctuation))
            gender = str(name[6]).translate(str.maketrans('','',string.punctuation))
            print("NAME:: ", str(user))
            self.ids['Player'].text = str(user)

            select_Icon = "ASSETS/PlayerIcon/"+str(icon)+str(gender[0])+".png"
            print("ICON??:\n    :", str(select_Icon))
            self.ids['Icon_'].text = str(select_Icon)
        except Exception as e:
...
#        MDApp.get_running_app().root.current = 'Login'
    ...
    def ads(self):
        ...
        if self.ads_count == 1:
            self.ads_bank += "ONE"
            self.FM.write_file("SOCKET_DATA/ADS_BANK.txt", self.ads_bank, "w")
        if self.ads_count == 2:
            self.ads_bank += "TWO"
            self.FM.write_file("SOCKET_DATA/ADS_BANK.txt", self.ads_bank, "w")
        if self.ads_count == 3:
...
class Register(Popup):
    ...
    def submit(self):
        ...
        email = "<"+str(self.ids['input_RegMail'].text)+">@"
        Icon = str(self.select_Icon(day, month))+"@"
        gender = str(self.gender)
        player_data = "PROFILE@"+name+day+"#"+month+"#"+year+"#"+Count+email+Icon+gender
        self.FM.write_file("SOCKET_DATA/Profile.txt", player_data, "w")

        player_profile = name+day+"#"+month+"#"+year+"#"+Count+email+Icon+gender
        self.FM.write_file("SOCKET_DATA/NAME.txt", player_profile, "w")

        time.sleep(1.5)
        feedBack = str(self.FM.read_file("SOCKET_DATA/Player.txt"))
        print("FEDBACK:: ", feedBack)
        if "LOGIN" in feedBack:
            print("LOGIN>>?")
            Welcome().open()
        if "WELCOME_NEW" in feedBack:
            print("PLAY NOOBIE VIDEO.. then shift to Home")
            MDApp.get_running_app().root.current ="Home"
...
class Loading(BoxLayout):
    ...
    def update_time(self, sec):
        ...
        if self.sec == 8:
            ...
            pl = self.FM.read_file("SOCKET_DATA/NAME.txt")
            if len(str(pl)) >= 4:
                Welcome().open()
            else:
                Login().open()

        ...
        while True:
            ...
            try:
                try:
                    ...
                    print("GET_MSG_EXCEPTION:: ", str(e))
                    data = str(self.sock.recv(64).encode())
                    print("^^ ERROR ^^ NO_LEN RECVD ^^")

                if "OOP_PROFILE" in data:
                    self.FM.write_file("SOCKET_DATA/OppData.txt", data, "w")

                if "LOGIN" in data or "NEW" in data:
                    print("WRITING ", str(data), "TO PLAYER.txt")
                    self.FM.write_file("SOCKET_DATA/Player.txt", data, "w")
                else:
                    self.E.wait()
            except Exception as e:
                print("[SOCKET CLOSED]")
    ...
    def send_msg(self):
        ...
        try:
            while True:
                ...
                if self.init_data != data and len(data) >= 6:
                    msg_len = len(data)
                ...
                if self.init_pl != data2:# and len(data2) >= 3:
                    msg_len2 = len(data2)
